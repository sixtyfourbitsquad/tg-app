...
import { getTelegramBotUsername, telegramPremiumUrl } from "@/lib/site";

const CONNECT_PENDING = "vaultx_connect_pending";
...
export default function ProfilePage() {
  ...
  const claimSession = useCallback(() => {
    return axios
      .post<{ linked: boolean; telegram_id?: string }>("/api/auth/claim")
      .then((r) => {
        if (r.data?.telegram_id) setClientTelegramId(String(r.data.telegram_id));
      })
      .catch(() => {});
  }, []);
  ...
  const fetchProfile = useCallback(() => {
    return axios
      ...
      .then((r) => {
        ...
        setProfile(d);
        if (d.telegram_id) {
          setClientTelegramId(d.telegram_id);
          try {
            sessionStorage.removeItem(CONNECT_PENDING);
          } catch {
            /* ignore */
          }
        }
  ...
  const refreshAll = useCallback(() => {
    void claimSession().finally(() => void fetchProfile());
  }, [claimSession, fetchProfile]);
  ...
  useEffect(() => {
    let pending = false;
    try {
      pending = sessionStorage.getItem(CONNECT_PENDING) === "1";
    } catch {
      pending = false;
    }
    if (!pending) return;
    ...
    const id = window.setInterval(() => {
      void refreshAll();
      const start = pollStartRef.current ?? 0;
      if (Date.now() - start > 120_000) window.clearInterval(id);
    }, 2500);
  ...
  const handleConnect = useCallback(async () => {
    setConnecting(true);
    try {
      const res = await axios.post<{ code: string }>("/api/auth/connect");
      try {
        sessionStorage.setItem(CONNECT_PENDING, "1");
      } catch {
        /* ignore */
      }
      pollStartRef.current = Date.now();
      window.location.href = `https://t.me/${getTelegramBotUsername()}?start=connect_${res.data.code}`;
    } catch {
      setConnecting(false);
    }
  }, []);
  ...
  const openPremium = useCallback(() => {
    window.open(telegramPremiumUrl(), "_blank", "noopener,noreferrer");
  }, []);
            ...
            <div>
              <div className="flex items-center gap-2">
                <p className="text-lg font-semibold">
                  {tgUser?.first_name ??
                    (profile.username !== "Anonymous" && profile.username !== "Guest"
                      ? `@${profile.username}`
                      : profile.username)}
                </p>
            ...
            <div className="flex items-center justify-between">
              ...
              {!(profile.is_premium || tgPremium) && (
                <button
                  type="button"
                  onClick={openPremium}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-gradient-to-r from-yellow-500 to-orange-500 text-black text-xs font-bold"
                >
